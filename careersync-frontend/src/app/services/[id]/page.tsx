...
export default function ServiceDetailPage({ params }: { params: Promise<{ id: string }> }) {
  ...
  if (!service) return (
    <div className="min-h-screen bg-slate-50">
      <Navbar />
      <div className="text-center py-10 pt-24 text-slate-900">Service not found.</div>
    </div>
  );
      ...
      <main className="container mx-auto py-8 pt-24 px-4">
        <div className="grid md:grid-cols-3 gap-8">
          <div className="md:col-span-2">
            ...
            <h1 className="text-4xl font-bold mt-2 text-slate-900">{service.name}</h1>
            <p className="text-lg mt-4 text-slate-600">{service.description}</p>
          </div>
          <div>
            <div className="rounded-lg border border-slate-200 bg-white p-6 shadow-lg">
              <h3 className="text-xl font-semibold text-slate-900">Offered by</h3>
